Vue.createApp({
  ...
  computed: {
    tableColumnHeadNames() {
      ...
      const columnResult = Object.keys(singleBook).filter((key) => {
  ...
  methods: {
    getDiscount(price, discount) {
      ...
      const priceNumber = parseInt(priceOnly);
      const discountValue = (priceNumber / 100) * discount;
      const discountPrice = (priceNumber - discountValue).toFixed(2);
